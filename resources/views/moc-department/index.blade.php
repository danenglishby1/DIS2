...
@section('pageTitle', 'MoC Department')
@section('pageName', 'MoC Department')
@section('css')
...
@section('content')
    ...
    <div class="row">
        <div class="col-sm-12">
            <a href="{{ route('moc-department.create')}}" class="btn btn-primary mb-2">Create New</a>
            ...
            <table id="macro-table" class="table table-striped">
                ...
                <tbody>
                @foreach($mocDepartments as $mocDepartment)
                    <tr>
                        <td>{{$mocDepartment->department}}</td>
                        <td>{{$mocDepartment->created_at}}</td>
                        <td>
                            <div style="display: flex;">

                                <a href="{{ route('moc-department.edit',$mocDepartment->id)}}" class="btn btn-warning m-1">Edit</a>
                                <form method="post" class="delete-form"
                                      action="{{ route('moc-department.destroy', $mocDepartment->id)}}">
                                    @csrf
                                    @method('DELETE')
