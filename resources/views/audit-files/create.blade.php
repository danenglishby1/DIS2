...
@section('pageTitle', 'Add Files To Audit')
@section('pageName', 'Add Files To Audit')
...
@section('content')
    ...
    @if(count($auditFiles) > 0)
            ...
            <table class="table">
                ...
                @foreach($auditFiles as $auditFile)
                    <tr>
                    <td><a target="_blank" href="{{asset('public/storage/audit-files/') ."/".$auditFile->file_path}}">{{$auditFile->original_file_name}}</a></td>
                    <td><form method="post" class="delete-form"
                              action="{{ route('audit-files.destroy', $auditFile->id)}}">
                            @csrf
                            @method('DELETE')
    ...
    <form action="{{route('audit-files.store')}}"
          class="dropzone"
          id="dropzone">
