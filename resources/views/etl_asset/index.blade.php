...
@section('content')
    ...
    <div class="row">
        <div class="col-sm-12">
            <a href="{{ route('etl_asset.create')}}" class="btn btn-primary mb-2">Create New</a>
            ...
            <table id="macro-table" class="table table-striped">
                ...
                <tbody>
                @foreach($etlAssets as $etlAsset)
                    <tr>
                        <td>{{$etlAsset->name}}</td>
                        <td>{{$etlAsset->user->name}}</td>
                        <td>{{$etlAsset->created_at}}</td>
                        <td>
                            <div style="display: flex;">


                                <a href="{{ route('etl_asset.show',$etlAsset->id)}}" class="btn btn-primary m-1">View</a>
{{--                                $userId == R.Butler || $userId == LABS || $userId == D.E--}}
{{--                                @if($userId == 47 || $userId == 56 || $userId == 4)--}}
                                <a href="{{ route('etl_asset.edit',$etlAsset->id)}}" class="btn btn-warning m-1">Edit</a>

                                <form method="post" class="delete-form"
                                      action="{{ route('etl_asset.destroy', $etlAsset->id)}}">
                                    @csrf
                                    @method('DELETE')
...
<script>
            ...
            tablesRows += "<tr>" +
                ...
                "<td>" +
                "  <div style='display: flex;'>" +
                "  <a href='"+rootUrl+"/etl_asset/"+data[key].id+"' class='btn btn-primary m-1'>View</a>" +
                "  <a href='"+rootUrl+"/etl_asset/"+data[key].id+"/edit' class='btn btn-warning m-1'>Edit</a>" +
                "<form method='post' class='delete-form' action='"+rootUrl+"/etl_asset/"+data[key].id+"'>" +
