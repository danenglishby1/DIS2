...
@section('pageTitle', 'Macros')
@section('pageName', 'All Macros')
        ...
        <div class="col-sm-12">
            ...
            <table id="macro-table" class="table table-striped">
                ...
                @foreach($macros as $macro)
                    <tr>
                        <td>{{str_pad($macro->WEEK_YEAR,2, "0", STR_PAD_LEFT) . str_pad($macro->COIL,3,"0",STR_PAD_LEFT)  . str_pad($macro->PIPE, 2, "0", STR_PAD_LEFT)}}</td>
                        <td>{{$macro->DIAM_RANGE}}</td>
                        <td>{{$macro->THK}}</td>
                        <td>{{ (isset($swerveLookupTable[$macro->SWERVE]) ? $swerveLookupTable[$macro->SWERVE] : "") }}</td>
                        <td>{{$macro->QUALITY}}</td>
                        <td>{{$macro->COMMENT}}</td>
                        ...
                        <td>
                            @if ($macro->IMAGE !== "")
                                <a class="macroImageLink" target="_blank"
                                   href="{{asset('public/storage/macros/'.$macro->IMAGE)}}"><img
                                        style="width:100px; height: 50px;"
                                        src="{{asset('public/storage/macros/'.$macro->IMAGE)}}"/></a>
                            @endif
                        </td>
                        <td>
                            <div style="display: flex;">
                                ...
                                <a href="{{ route('wm-macros.show',$macro->id)}}" class="btn btn-primary m-1">View</a>
{{--                                $userId == R.Butler || $userId == LABS || $userId == D.E    --}}
                                @if($userId == 47 || $userId == 56 || $userId == 4)
                                <a href="{{ route('wm-macros.edit',$macro->id)}}" class="btn btn-warning m-1">Edit</a>

                                <form method="post" class="delete-form"
                                      action="{{ route('wm-macros.destroy', $macro->id)}}">
                                    @csrf
                                    @method('DELETE')
