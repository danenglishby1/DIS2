...
@section('pageTitle', 'NCR')
@section('pageName', 'NCR')
...
@section('content')
    ...
    <div class="row">
        <div class="col-sm-12">
            <a href="{{ route('ncr.create')}}" class="btn btn-primary mb-2">Create New</a>
                    ...
                    <div id="doublescroll">
                        <table id="macro-table" class="table table-striped">
                            ...
                            @foreach($audits as $audit)
                                ...
                                <tr>
                                    <td>{{$audit->id}}</td>
                                    <td>{{$audit->MocDepartment->department}}</td>
                                    <td>{{$audit->auditProcessArea->name}}</td>

                                    <td>{{$audit->auditorUser->name}}</td>
                                    <td>{{$audit->managerResponsibleUser->name}}</td>
                                    @if(strlen($audit->comments) > 200)
                                        <td>{{substr($audit->comments, 0 , 200) . "......"}}</td>
                                    @else
                                    ...
                                    <td>
                                        <div style="display: flex;">
                                            <a href="{{ route('ncr.show',$audit->id)}}"
                                               class="btn btn-primary m-1">View</a>

                                            <a href="{{ route('ncr.edit',$audit->id)}}"
                                               class="btn btn-warning m-1">Edit</a>


                                            <a class="btn btn-success m-1"
                                               href="{{route('ncr-files.create').'?id='.$audit->id}}">Manage Files</a>

                                            <form method="post" class="delete-form"
                                                  action="{{ route('ncr.destroy', $audit->id)}}">
                                                @csrf
                                                @method('DELETE')
