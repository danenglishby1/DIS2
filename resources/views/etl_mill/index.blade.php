...
@section('pageTitle', 'ETL Mills')
@section('pageName', 'All Mills')
...
@section('content')
    ...
    <div class="row">
        <div class="col-sm-12">
            <a href="{{ route('etl_mill.create')}}" class="btn btn-primary mb-2">Create New</a>
            ...
            <table id="macro-table" class="table table-striped">
                ...
                @foreach($etlMills as $etlMill)
                    <tr>
                        <td>{{$etlMill->name}}</td>
                        <td>{{$etlMill->name}}</td>
                        <td>{{$etlMill->created_at}}</td>
                        <td>
                            <div style="display: flex;">


                                <a href="{{ route('etl_mill.show',$etlMill->id)}}" class="btn btn-primary m-1">View</a>
{{--                                $userId == R.Butler || $userId == LABS || $userId == D.E--}}
{{--                                @if($userId == 47 || $userId == 56 || $userId == 4)--}}
                                <a href="{{ route('etl_mill.edit',$etlMill->id)}}" class="btn btn-warning m-1">Edit</a>

                                <form method="post" class="delete-form"
                                      action="{{ route('etl_mill.destroy', $etlMill->id)}}">
                                    @csrf
                                    @method('DELETE')
...
<script>
            ...
            tablesRows += "<tr>" +
                ...
                "<td>" +
                "  <div style='display: flex;'>" +
                "  <a href='"+rootUrl+"/etl_mill/"+data[key].id+"' class='btn btn-primary m-1'>View</a>" +
                "  <a href='"+rootUrl+"/etl_mill/"+data[key].id+"/edit' class='btn btn-warning m-1'>Edit</a>" +
                "<form method='post' class='delete-form' action='"+rootUrl+"/etl_mill/"+data[key].id+"'>" +
