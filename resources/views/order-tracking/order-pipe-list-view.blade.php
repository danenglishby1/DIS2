...
@section('pageTitle', 'Order Pipe List')
@section('pageName', 'Order Pipe List')
        ...
        <table id="pipeList-tbl" class="table table-striped table-bordered" style="width:100%">
            ...
            @foreach($pipeListArray as $row)
                <tr>
                    ...
                    <td>{{$row["CAST_NO"]}}</td>
                    <td>{{$row["COIL_NO"]}}</td>
                    <td>{{$row["COIL_COIL_QUALITY"]}}</td>
                    <td>{{$row["PIPE_LENGTH"]}}</td>
                    <td>{{$row["T_WEIGHT"]}}</td>
                    <td>{{$row["ACTUAL_WEIGHT"]}}</td>
                    <td>{{$row["ORDER_POS"]}}</td>
                    <td>{{$row["DATETIME_TANDEM"]}}</td>
                    <td>{{$row["HAULIER_NAME"]}}</td>
...
{{--    </div>--}}
        ...
        {!! ($selectedOrderItem !== null ? '<a class="downloadButton" href="'. route('generate-tally-list').'?ORDER_ITEM='.$selectedOrderItem.'"><button class="btn btn-success mb-4 downloadButton" id="generateTallyButton" data-orderitem="'.$selectedOrderItem.'">Generate Tally List</button>' : '')!!}
...
@section('functionalScripts')
    ...
    <script>
        ...
        $('#orderItemSelectForm').submit(function(e){
            ...
            window.location.href = rootUrl + "/order-tracking/order-pipe-list-view?ORDER_ITEM=" + selectedOrder;
        });
