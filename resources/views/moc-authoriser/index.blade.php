...
@section('pageTitle', 'MoC Authoriser')
@section('pageName', 'MoC Authoriser')
@section('css')
...
@section('content')
    ...
    <div class="row">
        <div class="col-sm-12">
            <a href="{{ route('moc-authoriser.create')}}" class="btn btn-primary mb-2">Create New</a>
            ...
            <table id="macro-table" class="table table-striped">
                ...
                <tbody>
                @foreach($mocAuthoriser as $authoriser)
                    <tr>
                        <td>{{$authoriser->user->name}}</td>
                        <td>{{$authoriser->created_at}}</td>
                        <td>
                            <div style="display: flex;">


                                <a href="{{ route('moc-authoriser.edit',$authoriser->id)}}" class="btn btn-warning m-1">Edit</a>
                                <form method="post" class="delete-form"
                                      action="{{ route('moc-authoriser.destroy', $authoriser->id)}}">
                                    @csrf
                                    @method('DELETE')
