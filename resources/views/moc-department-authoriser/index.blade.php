...
@section('pageTitle', 'MoC Department Authorisers')
@section('pageName', 'MoC Department Authorisers')
...
@section('content')
    <div class="row">
        <div class="col-sm-12">
            <a href="{{ route('moc-department-authoriser.create')}}" class="btn btn-primary mb-2">Create New</a>
            ...
            <table id="macro-table" class="table table-striped">
                ...
                <tbody>
                @foreach($mocDepartmentAuthorisers as $deptAuthoriser)
                    <tr>
                        <td>{{$deptAuthoriser->MocAuthoriser->user->name}}</td>
                        <td>{{$deptAuthoriser->MocDepartment->department}}</td>
                        <td>{{$deptAuthoriser->created_at}}</td>

                        <td>
                            <div style="display: flex;">
                                <a href="{{ route('moc-department-authoriser.edit',$deptAuthoriser->id)}}" class="btn btn-warning m-1">Edit</a>
                                <form method="post" class="delete-form"
                                      action="{{ route('moc-department-authoriser.destroy', $deptAuthoriser->id)}}">
                                    @csrf
                                    @method('DELETE')
