...
@section('pageTitle', 'MoC Area Relations')
@section('pageName', 'MoC Area Relations')
@section('css')
...
@section('content')
    ...
    <div class="row">
        <div class="col-sm-12">
            <a href="{{ route('moc-area-relation.create')}}" class="btn btn-primary mb-2">Create New</a>
            ...
            <table id="macro-table" class="table table-striped">
                ...
                <tbody>
                @foreach($mocAreaRelations as $areaRelation)
                    <tr>
                        <td>{{$areaRelation->area_relation}}</td>
                        <td>{{$areaRelation->created_at}}</td>
                        <td>
                            <div style="display: flex;">


                                <a href="{{ route('moc-area-relation.edit',$areaRelation->id)}}" class="btn btn-warning m-1">Edit</a>
                                <form method="post" class="delete-form"
                                      action="{{ route('moc-area-relation.destroy', $areaRelation->id)}}">
                                    @csrf
                                    @method('DELETE')
