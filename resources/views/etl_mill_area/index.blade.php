...
@section('content')
    ...
    <div class="row">
        <div class="col-sm-12">
            <a href="{{ route('etl_mill_area.create')}}" class="btn btn-primary mb-2">Create New</a>
            ...
            <table id="macro-table" class="table table-striped">
                ...
                <tbody>
                @foreach($etlMillAreas as $etlArea)
                    <tr>

                        <td>{{$etlArea->mill->name}}</td>
                        <td>{{$etlArea->area->name}}</td>
...
{{--                                @if($userId == 47 || $userId == 56 || $userId == 4)--}}
                                <a href="{{ route('etl_mill_area.edit',$etlArea->id)}}" class="btn btn-warning m-1">Edit</a>

                                <form method="post" class="delete-form"
                                      action="{{ route('etl_mill_area.destroy', $etlArea->id)}}">
                                    @csrf
                                    @method('DELETE')
