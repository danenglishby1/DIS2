    ...
    <div>
        <table id="pando-list" class="table table-striped table-bordered" style="width:100%">
            ...
            @foreach($data as $row)
                ...
                @if($row["DaysLate"] == 1)
                    <tr style="background: #ffd1ac">
                @elseif($row["DaysLate"] > 1)
                    <tr style="background: #ff9f9f">
                @else
                    <tr>
                        @endif
                        <td>{{$row["Order_Number"]}}</td>
                        <td>
                            <a href="<?php echo $rootUrl; ?>/p_and_o/p_and_o_log?reference={{$row["Reference1"]}}">{{$row["Reference1"]}}</a>
                        </td>
                        @if($row["DaysLate"] >= 1)
                            <td>Y</td>
                        @else
                        ...
                        <td style="display: grid" data-id="{{$row["id"]}}" data-reference1="{{$row["Reference1"]}}"
                            data-reqdeldate="{{$row["Requested_Del_From_Date_Time"]}}">
                            {{$row["Comments"]}}
