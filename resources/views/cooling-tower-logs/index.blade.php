...
@section('pageTitle', 'Cooling Tower Logs')
@section('pageName', 'All Cooling Tower Logs')
...
{{--            </div>--}}
    ...
    <div class="row">
        <div class="col-sm-12">
            <a href="{{ route('cooling-tower-logs.create')}}" class="btn btn-primary mb-2">Create New</a>

            <table id="macro-table" class="table table-striped">
                <thead>
                <th>Cooling tower</th>
                <th>Is online and in operation</th>
                <th>Is water in CT sump fresh and clear</th>
                <th>Is the system operating as normal</th>
                ...
                <tbody>
                @foreach($coolingTowerLogs as $log)
                    <tr>

                        <td>{{$log->cooling_tower_identifier}}</td>
                        <td>{{$log->online}}</td>
                        <td>{{$log->fresh}}</td>
                        <td>{{$log->operating_normal}}</td>
                        ...
                        <td>
                            <div style="display: flex;">
                                <a href="{{ route('cooling-tower-logs.show',$log->id)}}" class="btn btn-primary m-1">View</a>

                                <a href="{{ route('cooling-tower-logs.edit',$log->id)}}" class="btn btn-warning m-1">Edit</a>

                                <form method="post" class="delete-form"
                                      action="{{ route('cooling-tower-logs.destroy', $log->id)}}">
                                    @csrf
                                    @method('DELETE')
...
<script>
            ...
            tablesRows += "<tr>" +
                ...
                "<td>" +
                "  <div style='display: flex;'>" +
                "  <a href='"+rootUrl+"/cooling-tower-logs/"+data[key].id+"' class='btn btn-primary m-1'>View</a>" +
                "  <a href='"+rootUrl+"/cooling-tower-logs/"+data[key].id+"/edit' class='btn btn-warning m-1'>Edit</a>" +
                "<form method='post' class='delete-form' action='"+rootUrl+"/cooling-tower-logs/"+data[key].id+"'>" +
