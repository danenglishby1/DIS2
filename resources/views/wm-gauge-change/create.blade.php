...
@section('content')
    ...
    <div class="row ml-3 mr-3">
        <a style="display: none;margin: auto;" id="setup-sheet" href="#">Set Up Sheet</a>
        ...
        <form method="post" id="gauge-change-form" action="{{ route('wm-gauge-change.store') }}"
            ...
            <div class="d-flex justify-content-center"
                ...
                <div class="form-group m-1">
                    ...
                    <input type="number" id="yearNo" maxlength="2" class="form-control" required name="year"/>
                </div>
                ...
                <div class="form-group m-1">
                    ...
                    <input type="text"  id="grade" class="form-control" required readonly
                           name="grade"/>
                </div>

                <div class="form-group m-1">
                    <label for="processRoute">PR</label>
                    <input type="string" id="processRoute" class="form-control" readonly required name="processRoute"/>
                </div>
...
{{--        {{asset('public/storage/wm-set-up-files/219.1.xlsx')}}       --}}
        ...
        $('#weekNo, #coilNo').focusout(function () {
            ...
            if (yearNo !== "" && weekNo !== "" && coilNo !== "" && weekNo.length >= 1 && coilNo.length >= 1 && yearNo.length >= 1) {
                ...
                $.ajax({
                    type: 'POST',
                    data: {"coilNo": coilNo, "weekNo": weekNo, "year" : yearNo},
                    async: false,
                ...
                if (!gotExistingGaugeChangeData) {
                    ...
                    $.ajax({
                        type: 'POST',
                        data: {"coilNo": coilNo, "week": weekNo, "year" : yearNo},
                        ...
                        success: function (data) {
                            ...
                            if (data.length == 0) {
                                ...
                                $('#grade').val("");
                                $('#processRoute').val("");
                                $('#setup-sheet').css('display', 'none');
                            } else {
                                $('#quality').val(data[0].COIL_COIL_QUALITY);
                                $('#diameter').val(data[0].BLOCK_OD);
                                // Populate set up sheet link
                                $('#setup-sheet').attr('href', rootUrl + "/public/storage/wm-set-up-files/" + data[0].BLOCK_OD + ".xlsx")
                                $('#setup-sheet').css('display', 'block');

                                $('#thickness').val(data[0].BLOCK_THICK);
                                $('#coilNotFoundAlert').css('display', 'none');
                                $('#grade').val(data[0].BLOCK_GRADE);
                                $('#processRoute').val(data[0].PROCESS_ROUTE);
                            }
                        },
                        complete: function () {
                            $('.ajax-loader').css('display', 'none');
